...
#include "ev_epoll.h"
static struct event_ops *cur_ops = &epoll_ops;
#elif defined(USED_SELECT)
#include "ev_select.h"
static struct event_ops *cur_ops = &select_ops;
#elif defined(USED_KQUEUE)
#include "ev_kqueue.h"
static struct event_ops *cur_ops = &kqueue_ops;
#else
#error "You must make one additional specification here "
#endif
...
evEventLoop *evCreateEventLoop(void)
{
    ...
    if(cur_ops->evops_ApiCreate(eventLoop) == -1) {
        free(eventLoop);
        return NULL;
    }
...
void evDeleteEventLoop(evEventLoop *eventLoop) {
    //evApiFree(eventLoop);
    cur_ops->evops_ApiFree(eventLoop);
    free(eventLoop);
}
        ...
        evFileProc *proc, void *clientData)
{
    ...
    if(cur_ops->evops_ApiAddEvent(eventLoop,fd,mask) == -1)
        return EVENT_ERR;
    fe->mask |= mask;
...
void evDeleteFileEvent(evEventLoop *eventLoop, int fd, int mask)
{
    ...
    cur_ops->evops_ApiDelEvent(eventLoop,fd,mask);
}
...
int evProcessEvents(evEventLoop *eventLoop, int flags)
{
    ...
    if (eventLoop->maxfd != -1 ||
        ...
        numevents = cur_ops->evops_ApiPoll(eventLoop,tvp);
